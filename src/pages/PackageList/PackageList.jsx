  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        console.warn(
          "Người dùng chưa đăng nhập, hiển thị danh sách gói dịch vụ."
        );
        setIsLoggedIn(false);
  ...
  const handleClick = () => {
    if (!isLoggedIn) {
      ...
      if (!packages || packages.length === 0) {
        message.error("Dữ liệu gói dịch vụ chưa tải xong!");
        return;
      }
      ...
      if (premiumPackage) {
        navigate(`/payment-detail/${premiumPackage.id}`);
      } else {
        message.error("Không tìm thấy gói Cao Cấp!");
      }
    }
  };
  const premiumPackage = packages.find((pkg) => pkg.type === "Premium");
  ...
  return (
    <div className="package-list">
      ...
      <header className="hero-section">
        ...
        <p>
          Chọn gói dịch vụ phù hợp để theo dõi thai kỳ một cách an toàn & khoa
          học.
        </p>
      </header>
      ...
      {currentPackage !== "Premium" && (
        <section className="cta-section">
          <h2>
            🌟 Chăm sóc thai kỳ toàn diện chưa bao giờ dễ dàng đến thế! 🌟
          </h2>
          <p>
            Nâng cấp ngay để tận hưởng đầy đủ tiện ích và sự hỗ trợ từ chuyên
            gia.
          </p>
      ...
      <section className="comparison-section">
        ...
        <Table
          dataSource={[
            {
              key: "1",
              feature: "Truy cập Blog Cộng Đồng 📝",
              free: "✔️",
              premium: "✔️",
            },
            {
              key: "2",
              feature: "Bình luận & Thảo luận 💬",
              free: "✔️",
              premium: "✔️",
            },
            {
              key: "3",
              feature: "Nhận tài liệu miễn phí 📄",
              free: "✔️",
              premium: "✔️",
            },
            {
              key: "4",
              feature: "Theo dõi Thai Nhi 📊",
              free: "❌",
              premium: "✔️",
            },
            {
              key: "5",
              feature: "Đặt lịch khám trực tuyến 🏥",
              free: "❌",
              premium: "✔️",
            },
            {
              key: "6",
              feature: "Nhắc nhở lịch khám 🔔",
              free: "❌",
              premium: "✔️",
            },
            {
              ...
              premium:
                currentPackage === "Premium" ? (
                  ...
                  <Button
                    ...
                    onClick={() =>
                      premiumPackage
                        ? handleBuyClick(premiumPackage.id)
                        : message.error("Không tìm thấy gói Cao Cấp!")
                    }
                  >
                    Nâng cấp ngay
